...
# NEW: container for survey simulation output
...
# NEW: Survey Simulation request/response models
class RunSurveyRequest(BaseModel):
    """
    Request body for survey simulation.

    questionnaire_blob_url: SAS URL to the questionnaire JSON (segment-based).
    audience_blob_url: SAS URL to the generated audience JSON.
    output_blob_prefix: prefix for the survey-answer output JSON blob name.
    """
...
class RunSurveyResponse(BaseModel):
    ...
    status: str
    output_blob: str  # SAS URL for survey-answer JSON
    total_members: int
    total_questions: int
    processing_time_seconds: float
...
def simulate_survey(req: RunSurveyRequest) -> RunSurveyResponse:
    ...
    try:
        ...
        results = run_survey_for_all_members(
            ...
            batch_size=30,  # fixed batch size as requested
        )
...
async def health_check() -> HealthResponse:
    ...
    return HealthResponse(
        status="ok",
        service="document-parser-and-audience-generator",
        version="1.0.0",
    )
